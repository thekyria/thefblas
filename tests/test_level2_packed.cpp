...
template <typename T>
std::vector<T> expand_packed(char uplo, int n, const std::vector<T>& ap) {
...
template <typename T>
std::vector<T> make_packed_real(char uplo, int n) {
  std::vector<T> ap(static_cast<std::size_t>(n) * (n + 1) / 2, T{});
  for (int j = 0; j < n; ++j) {
    const int first = (uplo == 'U') ? 0 : j;
    const int last = (uplo == 'U') ? j : (n - 1);
    for (int i = first; i <= last; ++i) {
...
void test_spmv_matches_symv() {
  ...
  for (const char uplo : {'U', 'L'}) {
    ...
    for (int i = 0; i < n; ++i) {
      assert(almost_equal_scalar(y_packed[i], y_dense[i]));
    }
  }
}
...
void test_hpmv_matches_hemv() {
  const int n = 3;
  for (const char uplo : {'U', 'L'}) {
    std::vector<cfloat> ap(static_cast<std::size_t>(n) * (n + 1) / 2, cfloat(0.0F, 0.0F));
    for (int j = 0; j < n; ++j) {
      const int first = (uplo == 'U') ? 0 : j;
      const int last = (uplo == 'U') ? j : (n - 1);
      for (int i = first; i <= last; ++i) {
  ...
  for (const char uplo : {'U', 'L'}) {
    ...
    for (const char diag : {'N', 'U'}) {
      for (const char trans : {'N', 'T', 'C'}) {
        ...
        for (int i = 0; i < n; ++i) {
          assert(almost_equal_scalar(x_packed[i], x_dense[i]));
        }

        thefblas::tpsv(uplo, trans, diag, n, ap.data(), x_packed.data(), 1);
        for (int i = 0; i < n; ++i) {
          assert(almost_equal_scalar(x_packed[i], x0[i], 1e-4));
        }
      }
    }
  }
...
void test_tpmv_negative_stride_and_complex() {
  ...
  for (int i = 0; i < 2; ++i) {
    assert(almost_equal_scalar(xc_packed[i], xc_dense[i]));
  }
  ...
  for (int i = 0; i < 2; ++i) {
    assert(almost_equal_scalar(xc_packed[i], before[i]));
  }
}
...
void test_spr_and_spr2_match_dense() {
  ...
  for (const char uplo : {'U', 'L'}) {
    ...
    for (int j = 0; j < n; ++j) {
      ...
      const int last = (uplo == 'U') ? j : (n - 1);
      for (int i = first; i <= last; ++i) {
        assert(almost_equal_scalar(ap[static_cast<std::size_t>(packed_index(uplo, i, j, n))],
