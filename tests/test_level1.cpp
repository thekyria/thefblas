...
template <typename T>
bool almost_equal(const T& lhs, const T& rhs, double tolerance = 1e-5) {
...
void test_real_fixed() {
  ...
  {
    const std::array<fixed32, 5> x = {fixed32(1.0), fixed32(9.0), fixed32(2.0), fixed32(9.0), fixed32(3.0)};
...
void test_complex_float() {
...

  {
    ...
    assert(almost_equal(thefblas::dotu(2, x.data(), 1, y.data(), 1), std::complex<float>(3.0F, 7.0F)));
    assert(almost_equal(thefblas::dotc(2, x.data(), 1, y.data(), 1), std::complex<float>(1.0F, -1.0F)));
  }
...
void test_complex_double() {
  const std::array<std::complex<double>, 5> x = {{{1.0, 1.0}, {9.0, 9.0}, {2.0, -1.0}, {9.0, 9.0}, {3.0, 0.5}}};
...
void test_complex_fixed() {
  ...
  {
    std::array<cfixed, 2> x = {cfixed(fixed32(1.0), fixed32(2.0)), cfixed(fixed32(3.0), fixed32(-1.0))};
    std::array<cfixed, 2> y = {cfixed(fixed32(2.0), fixed32(0.0)), cfixed(fixed32(0.0), fixed32(1.0))};
    assert(almost_equal(thefblas::dotu(2, x.data(), 1, y.data(), 1), cfixed(fixed32(3.0), fixed32(7.0)), 2e-3));
    assert(almost_equal(thefblas::dotc(2, x.data(), 1, y.data(), 1), cfixed(fixed32(1.0), fixed32(-1.0)), 2e-3));
  }

  {
    std::array<cfixed, 2> x = {cfixed(fixed32(3.0), fixed32(4.0)), cfixed(fixed32(1.0), fixed32(-2.0))};
