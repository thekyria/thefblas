...
std::vector<T> expand_general_band(int m, int n, int kl, int ku, const std::vector<T> &ab,
                                   int ldab) {
    std::vector<T> dense(static_cast<std::size_t>(m) * uz(n), T{});
    for (int j = 0; j < n; ++j) {
        const int first = (j - ku > 0) ? (j - ku) : 0;
        const int last = (j + kl < m - 1) ? (j + kl) : (m - 1);
        for (int i = first; i <= last; ++i) {
            dense[static_cast<std::size_t>(i) + static_cast<std::size_t>(j) * uz(m)] =
                ab[static_cast<std::size_t>(ku + i - j) + static_cast<std::size_t>(j) * uz(ldab)];
        }
    }
    return dense;
}
...
template <typename T>
std::vector<T> expand_triangle_band(char uplo, int n, int k, const std::vector<T> &ab, int ldab) {
        ...
        if (uplo == 'U') {
            ...
            for (int i = first; i <= j; ++i) {
                dense[static_cast<std::size_t>(i) + static_cast<std::size_t>(j) * uz(n)] =
                    ab[static_cast<std::size_t>(k + i - j) +
                       static_cast<std::size_t>(j) * uz(ldab)];
            }
        } else {
            const int last = (j + k < n - 1) ? (j + k) : (n - 1);
            for (int i = j; i <= last; ++i) {
                dense[static_cast<std::size_t>(i) + static_cast<std::size_t>(j) * uz(n)] =
                    ab[static_cast<std::size_t>(i - j) + static_cast<std::size_t>(j) * uz(ldab)];
            }
        }
    }
    return dense;
...
void test_gbmv_float_matches_dense() {
    ...
    std::vector<float> ab(static_cast<std::size_t>(ldab) * uz(n), 0.0F);
    for (int j = 0; j < n; ++j) {
        const int first = (j - ku > 0) ? (j - ku) : 0;
    ...
    for (const char trans : {'N', 'T', 'C'}) {
        ...
        thefblas::gbmv(trans, n, n, kl, ku, 2.0F, ab.data(), ldab, x.data(), 1, 0.5F, y_band.data(),
                       1);
        thefblas::gemv(trans, n, n, 2.0F, dense.data(), n, x.data(), 1, 0.5F, y_dense.data(), 1);
        for (int i = 0; i < n; ++i) {
            assert(almost_equal_scalar(y_band[uz(i)], y_dense[uz(i)]));
        }
    }
}
...
void test_gbmv_rectangular_negative_stride() {
    ...
    thefblas::gbmv('N', m, n, kl, ku, 1.0F, ab.data(), ldab, x.data(), -1, 0.0F, y_band.data(), -1);
...
void test_gbmv_fixed_and_invalid_args() {
    ...
    const int n = 3;
    const int ldab = 3;
    std::vector<fixed32> ab(static_cast<std::size_t>(ldab) * uz(n), fixed32(0));
    for (int j = 0; j < n; ++j) {
        ab[static_cast<std::size_t>(1) + static_cast<std::size_t>(j) * uz(ldab)] = fixed32(2);
    }
    const std::array<fixed32, 3> x = {fixed32(1), fixed32(2), fixed32(3)};
    std::array<fixed32, 3> y = {fixed32(0), fixed32(0), fixed32(0)};
    thefblas::gbmv('N', n, n, 1, 1, fixed32(1), ab.data(), ldab, x.data(), 1, fixed32(0), y.data(),
                   1);
    ...
    thefblas::gbmv('X', n, n, 1, 1, fixed32(1), ab.data(), ldab, x.data(), 1, fixed32(1), y.data(),
                   1);
    thefblas::gbmv('N', n, n, -1, 1, fixed32(1), ab.data(), ldab, x.data(), 1, fixed32(1), y.data(),
                   1);
    thefblas::gbmv('N', n, n, 1, 1, fixed32(1), ab.data(), 1, x.data(), 1, fixed32(1), y.data(), 1);
    assert(almost_equal_scalar(y[0], fixed32(2)));
    assert(almost_equal_scalar(y[1], fixed32(4)));
    assert(almost_equal_scalar(y[2], fixed32(6)));
}
...
void test_sbmv_matches_symv() {
    ...
    for (const char uplo : {'U', 'L'}) {
        ...
        for (int j = 0; j < n; ++j) {
            if (uplo == 'U') {
                ab[static_cast<std::size_t>(k) + static_cast<std::size_t>(j) * uz(ldab)] =
                    static_cast<float>(j + 1);
                if (j > 0) {
                    ab[static_cast<std::size_t>(k - 1) + static_cast<std::size_t>(j) * uz(ldab)] =
                        0.5F;
                }
            } else {
                ab[static_cast<std::size_t>(j) * uz(ldab)] = static_cast<float>(j + 1);
                if (j < n - 1) {
                    ab[static_cast<std::size_t>(1) + static_cast<std::size_t>(j) * uz(ldab)] = 0.5F;
                }
            }
        }
...
void test_hbmv_matches_hemv() {
    ...
    const int ldab = k + 1;
    for (const char uplo : {'U', 'L'}) {
        std::vector<cfloat> ab(static_cast<std::size_t>(ldab) * uz(n), cfloat(0.0F, 0.0F));
        for (int j = 0; j < n; ++j) {
            if (uplo == 'U') {
                ab[static_cast<std::size_t>(k) + static_cast<std::size_t>(j) * uz(ldab)] =
                ...
                ab[static_cast<std::size_t>(j) * uz(ldab)] =
                    cfloat(static_cast<float>(j + 1), 0.0F);
                if (j < n - 1) {
                    ab[static_cast<std::size_t>(1) + static_cast<std::size_t>(j) * uz(ldab)] =
                        cfloat(0.5F, -1.0F);
                }
            }
        }
...
void test_tbmv_and_tbsv_match_dense() {
    ...
    for (const char uplo : {'U', 'L'}) {
        for (const char diag : {'N', 'U'}) {
            ...
            for (int j = 0; j < n; ++j) {
                if (uplo == 'U') {
                    ab[static_cast<std::size_t>(k) + static_cast<std::size_t>(j) * uz(ldab)] = 2.0F;
                    if (j > 0) {
                        ab[static_cast<std::size_t>(k - 1) +
                           static_cast<std::size_t>(j) * uz(ldab)] = 0.5F;
                    }
                } else {
                    ab[static_cast<std::size_t>(j) * uz(ldab)] = 2.0F;
                    if (j < n - 1) {
                        ab[static_cast<std::size_t>(1) + static_cast<std::size_t>(j) * uz(ldab)] =
                            0.5F;
                    }
...
void test_tbmv_negative_stride_and_fixed() {
    ...
    for (int j = 0; j < n; ++j) {
        ab[static_cast<std::size_t>(j) * uz(ldab)] = 2.0F;
        if (j < n - 1) {
            ab[static_cast<std::size_t>(1) + static_cast<std::size_t>(j) * uz(ldab)] = 1.0F;
        }
    }
