...
template <typename T>
struct accumulator<std::complex<T>> {
...
template <typename T>
inline accumulator_t<T> to_accumulator(const T& value) {
...
template <typename T>
inline T from_accumulator(const accumulator_t<T>& value) {
  if constexpr (is_complex_v<T>) {
    using R = typename T::value_type;
...
template <typename T>
inline accumulator_t<T> acc_mul(const T& a, const T& b) {
...
template <typename T>
class real_mac<T, typename std::enable_if<is_fixed_v<T>>::type> {
